...
import MPC_WRAPPER as TTMPC_CPP
from .robot_model import RobotModel

class TTMPC():
    def __init__(self) -> None:
        config_path = os.path.join(TTMPC_CPP.pkg_path, "Params/config.json")
        with open(config_path, 'r') as iConfig:
            self.jsonConfig = json.load(iConfig)
        self.json_paths = TTMPC_CPP.PathToJson()
        self.json_paths.param_path = os.path.join(TTMPC_CPP.pkg_path, self.jsonConfig["model_path"])
        self.json_paths.cost_path = os.path.join(TTMPC_CPP.pkg_path, self.jsonConfig["cost_path"])
        self.json_paths.bounds_path = os.path.join(TTMPC_CPP.pkg_path, self.jsonConfig["bounds_path"])
        self.json_paths.track_path = os.path.join(TTMPC_CPP.pkg_path, self.jsonConfig["track_path"])
        self.json_paths.normalization_path = os.path.join(TTMPC_CPP.pkg_path, self.jsonConfig["normalization_path"])
        self.json_paths.sqp_path = os.path.join(TTMPC_CPP.pkg_path, self.jsonConfig["sqp_path"])

        self.Ts = self.jsonConfig["Ts"]
        self.pred_horizon = TTMPC_CPP.N
        self.robot_dof = TTMPC_CPP.PANDA_DOF

        self.robot_model = RobotModel()
        self.num_links = TTMPC_CPP.PANDA_NUM_LINKS

        self.mpc = TTMPC_CPP.MPC(self.Ts, self.json_paths)
    ...
    def setParam(self, param_value:dict) -> None:
        ...
        param_value_cpp = TTMPC_CPP.ParamValue()
    ...
    def setTrack(self, state:np.array) -> None:
        assert state.size == TTMPC_CPP.NX, f"State size {state.size} does not match expected size {TTMPC_CPP.NX}"
        self.init_state = state

        ee_pos = self.robot_model.getEEPosition(self.init_state[:self.robot_model.num_q])
        track = TTMPC_CPP.Track(self.json_paths.track_path)
    ...
    def runMPC(self, state:np.array, input:np.array, time_idx:int, obs_position: np.array = np.array([3,3,3]), obs_radius: float = 0) -> {bool, np.array, np.array, list, dict}:
        assert self.track_set == True, "Set Track first!"
        assert state.size == TTMPC_CPP.NX, f"State size {state.size} does not match expected size {TTMPC_CPP.NX}"
        x0 = TTMPC_CPP.vectorToState(state)
        u0 = TTMPC_CPP.vectorToInput(input)
        mpc_sol = TTMPC_CPP.MPCReturn()
        # mpc_status = self.mpc.runMPC_(mpc_sol, x0, u0, voxel, time_idx)
        mpc_status = self.mpc.runMPC_(mpc_sol, x0, u0, time_idx, obs_position, obs_radius)
        updated_state = TTMPC_CPP.stateToVector(x0)

        mpc_horizon=[]
        for mpc_horizon_raw in mpc_sol.mpc_horizon:
            state_k = TTMPC_CPP.stateToVector(mpc_horizon_raw.xk)
            input_k = TTMPC_CPP.inputToVector(mpc_horizon_raw.uk)
        ...
        return mpc_status, updated_state, TTMPC_CPP.inputToVector(mpc_sol.u0), mpc_horizon, compute_time
