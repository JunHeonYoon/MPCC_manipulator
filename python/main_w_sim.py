...
def main(args):
    ...
    for time_idx in range(10000):
        ...
        q = state[:robot_dof]
        q_dot = state[robot_dof:2*robot_dof]
        q_ddot = opt_input[:robot_dof]
        s = state[-2]
        vs = state[-1]
        dVs = opt_input[-1]

        ee_pos = robot.getEEPosition(q)
        ee_ori = robot.getEEOrientation(q)
        ee_vel = robot.getEEJacobianv(q) @ q_dot
        min_dist, _ = selcolNN.calculateMlpOutput(q)
        mani = robot.getEEManipulability(q)
        real_min_dist = pc.min_distance(q)*100
        ...
        print("dVs         : {:0.5f}".format(dVs))
        print("Terminal con: {0:0.3f}, {1:0.3f}, {2:0.3f}".format(np.linalg.norm((spline_pos[-1] - ee_pos), 2), np.linalg.norm(mpcc.Log(spline_ori[-1].T @ ee_ori), 2),abs(s - spline_arc_length[-1])))
        ...
        if np.linalg.norm((spline_pos[-1] - ee_pos), 2) < 1E-2 and np.linalg.norm(mpcc.Log(spline_ori[-1].T @ ee_ori), 2) < 1E-2 and abs(s - spline_arc_length[-1]) < 1E-2:
    ...
    with open('debug.txt', 'w') as debug_file:
        for q, qdot, qddot, min_dist, mani, pred_ee_pose, ref_ee_pose in zip(debug_data["q"], debug_data["qdot"], debug_data["qddot"], debug_data['min_dist'], debug_data['mani'], debug_data['pred_ee_pose'], debug_data['ref_ee_pose']):
            data_to_write = np.concatenate([q, qdot, qddot, np.array([min_dist, mani]), pred_ee_pose.flatten(), ref_ee_pose.flatten()], axis=0)
