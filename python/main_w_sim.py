...
def main(args):
    ...
    spline_T = np.zeros([total_ref_traj_posi.shape[0], 4, 4])
    for i, (posi, rot) in enumerate(zip(total_ref_traj_posi, total_ref_traj_ori)):
        spline_T[i, :3, :3] = rot
        spline_T[i, :3, 3] = posi
    ...
    while not rospy.is_shutdown():
        ...
        ## run MPCC
        status, state, input, mpc_horizon, compute_time = mpc.runMPC(state, input, time_idx, obs_position, obs_radius)
        if status == False:
        ...
        s, contour_error = mpc.getContourError(time_idx * mpc.Ts * param_value["param"]["desired_ee_velocity"], x)
        vs = param_value["param"]["desired_ee_velocity"]
        ...
        for i in range(mpc.pred_horizon + 1):
            ...
            ref_ee_T[i, :3, 3], ref_ee_T[i, :3, :3] = ref_n_traj_posi[i],ref_n_traj_ori[i]
        ...
        ## End condition 
        if np.linalg.norm((total_ref_traj_posi[-1] - x), 2) < 1E-2 and np.linalg.norm(TTMPC.Log(total_ref_traj_ori[-1].T @ rotation), 2) < 1E-2 and (time_idx > total_ref_traj_posi.shape[0]):
